...
TEST(search, all_basic){
    for (auto& tc : search_test_cases){
        ...
        for (auto& dfa : dfas){
            for (auto& good : tc.good){
                EXPECT_TRUE(search(good, dfa)) << "DEFAULT SEARCH Good test: \"" << good  << "\", for regex: \"" << test << "\""; 
            }
            for (auto& bad : tc.bad){
                EXPECT_FALSE(search(bad, dfa)) << "DEFAULT SEARCH Bad test: \"" << bad << "\", for regex: \"" << test << "\""; 
            }
        }
    }
}
